...
<article
  class="relative p-8 | flex flex-col items-center text-center | {border} rounded-b bg-blue-gray-800">
  ...
  <div class="flex flex-col gap-2">
    <b class="text-5xl text-white"> {stat}</b>
  ...
  <div class="mt-8 | flex items-center | text-sm font-bold {isPositive ? 'text-lime' : 'text-red'}">
    <i class="bx bx-chevron-{iconDirection}" />
    <span>
      {todayStat} Today
    </span>
  </div>
...
<style>
  article::after {
    content: '';
    @apply absolute top-0 left-0 right-0 h-1 rounded-t;
  }
</style>
